...
/**
 * Perform InverseSubBytes operation for AES-128 decryption
 *
 * @param input the input string, in hexadecimal number system
 * @returns the output after inverse byte substitution, as a string in hexadecimal number system
 */
string inverse_s_box_substitution(string input) 
{
    string inverse_s_box[16][16] = {"52", "09", "6A", "D5", "30", "36", "A5", "38",
    ...
    for(int i = 0; i < input.size() / 2; i++) {
        string substring = input.substr(i * 2, 2);
        char row_char = substring[0];
        char col_char = substring[1];
        int row = hex_to_dec(row_char); 
        int col = hex_to_dec(col_char);
        output += inverse_s_box[row][col];
    }  
    return output;                      
}